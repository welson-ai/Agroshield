...
contract DynamicPremiums is Ownable, ReentrancyGuard {
    ...
    function calculatePremium(
        ...
        uint256 _rainfallThreshold,
        uint256 _measurementPeriod
    ) external view returns (uint256 finalPremium) {
    ...
    function updateRiskFactor(
        ...
        riskFactors[_location].baseRiskScore = _baseRiskScore;
        riskFactors[_location].historicalRainfallVariance = _variance;
        riskFactors[_location].droughtFrequency = _droughtFreq;
        riskFactors[_location].floodFrequency = _floodFreq;
        riskFactors[_location].lastUpdated = block.timestamp;
        
        emit RiskFactorUpdated(_location, _baseRiskScore, _variance, _droughtFreq, _floodFreq);
    }
    
    function deactivateRiskFactor(string memory _location) external onlyOwner onlyValidOracle {
        require(riskFactors[_location].isActive, "Risk factor already inactive");
        
        riskFactors[_location].isActive = false;
    }
}
